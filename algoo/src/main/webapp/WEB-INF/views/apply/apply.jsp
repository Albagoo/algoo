...
<body>
	<form action="<c:url value='/apply/apply.ag'/>" method="post" id="frmApply">
		...
		이력서 목록 : <select class="textBox" style="margin-bottom: 10px" name="hisCode">
			<c:forEach var="vo" items="${alist }">
				<option value="${vo.hisCode }">${vo.title }</option>
			</c:forEach>
		</select>
		<p style="margin-bottom: 0">지원하기 전, 마지막으로 고용주에게 보낼 메모를 남겨주세요!</p>
		<textarea cols="5" id="memo" name="memo" placeholder="지원하기 전, 마지막으로 고용주에게 보낼 메모를 남겨주세요!"
				    	rows="80" style="width:500px; height:180px; 
